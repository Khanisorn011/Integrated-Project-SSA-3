  ...
  <div class="p-4 bg-gray-100 rounded-xl shadow-md flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    ...
    <div class="join flex flex-wrap items-center gap-2" v-show="totalPages > 1">
      ...
      <button
        ...
        :class="[`itbms-page-${page} join-item btn btn-sm px-3`, currentPage == page && 'btn-primary']"
      >
        {{ page + 1 }}
      </button>

      <button @click="$emit('update:currentPage', currentPage + 1)" :disabled="currentPage === totalPages - 1" class="itbms-page-next join-item btn btn-sm px-4">Next</button>
      <button @click="$emit('update:currentPage', totalPages - 1)" :disabled="currentPage === totalPages - 1" class="itbms-page-last join-item btn btn-sm px-4">Last</button>
    </div>
